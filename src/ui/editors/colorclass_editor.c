...
colorclass_viewer_add(Evas_Object *parent)
{
   ...
   evas_object_smart_callback_add(button, "clicked", _on_btn_add, mwin);
   elm_box_pack_end(bottom_box, button);

   BUTTON_ADD(mwin, button, "Delete");
   evas_object_smart_callback_add(button, "clicked", _on_btn_del, mwin);
   elm_box_pack_end(bottom_box, button);

   BUTTON_ADD(mwin, button, "Apply");
   evas_object_smart_callback_add(button, "clicked", _on_btn_apply, mwin);
   elm_box_pack_end(bottom_box, button);

   BUTTON_ADD(mwin, button, "Close");
   evas_object_smart_callback_add(button, "clicked", _on_btn_cancel, mwin);
   elm_box_pack_end(bottom_box, button);
...
#define _COLOR_ADD(rect, title) \
   ...
   _SPINNER_ADD(window.obj_r,"R:%3.0f")
   _SPINNER_ADD(window.obj_g,"G:%3.0f")
   _SPINNER_ADD(window.obj_b,"B:%3.0f")
   _SPINNER_ADD(window.obj_a,"A:%3.0f")
   _COLOR_ADD(window.rect_color2,"Outline color")
   _SPINNER_ADD(window.out_r,"R:%3.0f")
   _SPINNER_ADD(window.out_g,"G:%3.0f")
   _SPINNER_ADD(window.out_b,"B:%3.0f")
   _SPINNER_ADD(window.out_a,"A:%3.0f")
   _COLOR_ADD(window.rect_color3,"Shadow color")
   _SPINNER_ADD(window.sdw_r,"R:%3.0f")
   _SPINNER_ADD(window.sdw_g,"G:%3.0f")
   _SPINNER_ADD(window.sdw_b,"B:%3.0f")
   _SPINNER_ADD(window.sdw_a,"A:%3.0f")
