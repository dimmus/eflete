...
comment_apply(Eina_Strbuf *strbuf, const char **src, int length, char **cur,
              char **prev, const Eina_Stringshare *color_string,
...
string_apply(Eina_Strbuf *strbuf, char **cur, char **prev,
             const Eina_Stringshare *color_string, Eina_Bool inside_string)
{
   //escape string: " ~ "
   ...
   if (!inside_string)
     snprintf(buf, sizeof(buf), "<color=#%s>%s", color_string, QUOT);
   else
...
macro_apply(Eina_Strbuf *strbuf, const char **src, int length, char **cur,
            char **prev, const Eina_Stringshare *color_string, color_data *cd)
{
   if ((*cur)[0] != '#') return 0;
   ...
   else
   {
     ...
     while ((index >= 0) && (index < macro_len))
       {
          if (macro[index] == '(') bracket_inside++;
          else if (macro[index] == ')') bracket_inside--;
          index--;
       }
     if (bracket_inside > 0)
       {
