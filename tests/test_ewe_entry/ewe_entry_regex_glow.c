...
{
   ewe_init(0, 0);
   ck_assert_msg(ewe_entry_regex_glow_set(NULL, EINA_TRUE) == EINA_FALSE, "EINA_FALSE is expected");
   ewe_shutdown();
}
END_TEST
...
{
   ewe_init(0, 0);
   ck_assert_msg(ewe_entry_regex_glow_get(NULL) == EINA_FALSE, "EINA_FALSE is expected");
   ewe_shutdown();
}
END_TEST
...
EFLETE_TEST(ewe_entry_regex_glow_p1)
{
   ...
   ck_assert_msg(ewe_entry_regex_glow_get(obj) == EINA_FALSE, "Default visibility value is not correct");
   ewe_shutdown();
}
END_TEST
...
EFLETE_TEST(ewe_entry_regex_glow_p2)
{
   ...
   Evas_Object *win = elm_win_util_standard_add("entry", "Entry");
   Evas_Object *obj = ewe_entry_add(win);
   ck_assert_msg(ewe_entry_regex_glow_set(obj, EINA_TRUE) == EINA_TRUE, "ewe_entry_regex_glow_set() should return EINA_TRUE, but it did not");
   ck_assert_msg(ewe_entry_regex_glow_get(obj) == EINA_TRUE, "ewe_entry_regex_glow_get() should return EINA_TRUE, but it did not");
   ewe_shutdown();
}
END_TEST
